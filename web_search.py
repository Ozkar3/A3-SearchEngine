...
# Used to load the search engine as soon as it starts
GLOBAL_SEARCH_ENGINE: Optional[SearchEngine] = None
...
class SearchRequestHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self) -> None:
        if GLOBAL_SEARCH_ENGINE is None:
            self.send_error(500, "Search engine not initialized")
            return
        ...
        if query:
            ...
            results = GLOBAL_SEARCH_ENGINE.search(query, top_k=5)
...
def run_server() -> None:
    ...
    GLOBAL_SEARCH_ENGINE = SearchEngine(INDEX_DIR)
